...
async function measure(dir) {
  ...
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    ...
    const ar = (bw * w) / (bh * h); // pixel aspect of the blob
    // a punched hang-hole: a compact, roughly round blob, fully INTERIOR (not a
    // top-edge notch or an edge-hugging art region), in the upper part of the tab
    if (area > 0.012 && area < 0.16 && cy < 0.55 && bw > 0.05 && bw < 0.5 && fill > 0.6 && ar > 0.4 && ar < 2.5 && !touchBorder) {
      const score = fill - cy;
...
(async () => {
  ...
  for (const g of list) {
    ...
    g.box.flap.cornerR = 0.28; // round the tab's free (top) corners
    const hole = await measure(texDir(g.id));
